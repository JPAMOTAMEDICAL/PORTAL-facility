...
async function proxyRequest(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "API service is unavailable. Confirm the backend server is running and try again.",
      },
      { status: 503 },
    );
  }
...
export { proxyRequest as GET, proxyRequest as POST, proxyRequest as PUT, proxyRequest as PATCH, proxyRequest as DELETE, proxyRequest as HEAD, proxyRequest as OPTIONS };
